...
RSpec.describe ContactsController, type: :controller do
  describe 'Controller Action' do
    ...
    it 'should create contact' do
      post :create, {params: { contact: { email: 'tester@tester.com', first_name: 'John', last_name: 'Doe', message: 'test_message'}}}
      expect(response).to redirect_to contact_path
      expect(flash[:notice]).to eq('Contact created successfully!')
    end

    it 'should not create contact' do
      post :create, {params: { contact: { email: 'invalidemail', first_name: 'John', last_name: 'Doe', message: 'test_message'}}}
